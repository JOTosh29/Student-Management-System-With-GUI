...
class MainWindows(QMainWindow, Ui_MainWindow):
    ...
    def openShifterWindow(self):
        if self.shifterWindow is None:
            ...
            self.shifterWindow.pushButton_12.clicked.connect(self.shifterWindow.update_record)
    ...
    def openAdmissionWindow(self):
        if self.admissionWindow is None:
            ...
            self.admissionWindow.pushButton_12.clicked.connect(self.admissionWindow.update_record)
    ...
    def openTransferWindow(self):
        if self.transferWindow is None:
            ...
            self.transferWindow.pushButton_12.clicked.connect(self.transferWindow.update_record)
...
class ShifterWindow(QMainWindow):
    def __init__(self):
        ...
        self.pushButton_12.clicked.connect(self.update_record)
    ...
    def update_record(self):
        ...
        if selected_row >= 0:
            ...
            if student_code and last_name and first_name and middle_name and course and year:
                old_student_code = self.tableWidget.item(selected_row, 0).text()
                query = """UPDATE shifters 
                           SET student_code = %s, last_name = %s, first_name = %s, middle_name = %s, course = %s, year = %s 
                           WHERE student_code = %s"""
                params = (student_code, last_name, first_name, middle_name, course, year, old_student_code)
                db_manager.execute_query(query, params)
                self.load_table_data()
                QMessageBox.information(self, "Update Successful", "Record updated successfully.")
            else:
                QMessageBox.warning(self, "Input Error", "Please fill in all fields.")
        else:
...
class AdmissionWindow(QMainWindow):
    ...
    def update_record(self):
        ...
        if selected_row >= 0:
            ...
            if student_code and last_name and first_name and middle_name and course and year:
                old_student_code = self.tableWidget.item(selected_row, 0).text()
                query = """UPDATE admitted_students 
                           SET student_code = %s, last_name = %s, first_name = %s, middle_name = %s, course = %s, year = %s 
                           WHERE student_code = %s"""
                params = (student_code, last_name, first_name, middle_name, course, year, old_student_code)
                db_manager.execute_query(query, params)
...
class TransferWindow(QMainWindow):
    ...
    def update_record(self):
        ...
        if selected_row >= 0:
            ...
            if student_code and last_name and first_name and middle_name and course and year:
                query = """UPDATE transfer_students 
                           SET last_name = %s, first_name = %s, middle_name = %s, course = %s, year = %s 
                           WHERE student_code = %s"""
                params = (last_name, first_name, middle_name, course, year, student_code)
                db_manager.execute_query(query, params)
                self.load_table_data()
            else:
                QMessageBox.warning(self, "Input Error", "Please fill in all fields.")
